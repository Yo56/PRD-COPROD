...
vector<Chantier> Acquisition::chantiers(string path)
{
	rapidcsv::Document doc(path, rapidcsv::LabelParams(0, -1), rapidcsv::SeparatorParams(';'));
	vector<Chantier> listeChantiers;
	
	// NOMS DES COLONNES
	vector<string> nomsColonnes = doc.GetColumnNames();
	...
	for (int i = 0; i < col1.size(); i++)
	{
		...
		Chantier chantier(SR, libelleSR);
		cout << chantier << endl;
